        ...
        <div className='md:py-[23px] sm:py-5 py-[14px]'>
            <div className="container">
                <nav className='flex items-center justify-between'>
                    <a href="/" className='md:max-w-[180px] max-w-[117px] z-[90] flex'>
                        <img src={logo} alt="logo" className='w-full nav-gsap -translate-y-16 opacity-0 pointer-events-none'/>
                    </a>
                    <div className={`flex md:flex-row duration-300 max-md:pt-[108px] md:justify-center ease-linear bg-white z-50 flex-col items-center gap-[49px] md:gap-11 absolute top-0 -right-full w-full min-h-screen md:w-fit md:bg-transparent md:relative md:min-h-fit md:right-auto md:top-auto ${show ? "" : " !right-0"}`}>
                        {NAV_ITEMS_DATA.map((items, index) => (
                            <NavLink
                                ...
                                key={index}
                                className={({ isActive }) =>
                                    `nav-gsap -translate-y-16 opacity-0 text-gray hover:text-black text-base leading-6 font-normal duration-300 ease-linear after:left-[50%] after:translate-x-[-50%] after:mx-auto hover:after:w-[31px] after:h-[3px] after:absolute after:bg-black after:bottom-[-5px] after:w-0 after:rounded after:duration-300 after:ease-linear relative ${isActive ? 'text-black font-bold after:w-[31px]' : ''}`
                                }
                            >
                                {items.text}
                            </NavLink>
